(function () {
    ...
    function $sendBadjs(msg, src, d) {
        var ua = navigator.userAgent.toLowerCase(),
            bom = window.ActiveXObject ? "IE" + ua.match(/msie ([\d.]+)/)[1] : document.getBoxObjectFor ? "Firefox" + ua.match(/firefox\/([\d.]+)/)[1] : (ua.indexOf("chrome") != -1) ? "Chrome" + ua.match(/chrome\/([\d.]+)/)[1] : window.opera ? "Opera" + ua.match(/opera.([\d.]+)/)[1] : window.openDatabase ? "Safari" + ua.match(/version\/([\d.]+)/)[1] : ua.replace(/\s*/g, ""),
            ts = msg.replace(/\n/g, " ") + "|" + encodeURIComponent(src) + "|" + d;
        setTimeout(function () {
...
var FCAPP = FCAPP || {
    Common: {
        ...
        loadShareData: function (id) {
            var dt = new Date(),
                base = 'http://trade.qq.com/fangchan/static/',
                param = '' + dt.getMonth() + dt.getDate(),
                nid = '';
            id = (id + '').replace(/[<>\'\"\/\\&#\?\s\r\n]+/gi, '');
            nid = id.split(/[^\d]+/)[0];
            $.ajax({
        ...
        msg: function (boo, obj, isNotEscape) {
            ...
            msg = !! isNotEscape ? obj.msg : FCAPP.Common.escapeHTML(obj.msg);
            R.tipsMsg.html(msg);
        ...
        errJump: function (page) {
            ...
            var baseInfo = {
                ...
                title: gQuery.title || '',
                _: Math.random()
            }, url = 'http://trade.qq.com/fangchan/' + page + '.html?' + $.param(baseInfo),
                jumpUrl = 'http://cgi.trade.qq.com/cgi-bin/common/wx_redirect.fcg?';
