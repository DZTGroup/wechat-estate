...
FCAPP.PICWALL = {
    ...
    updateImgMargin: function () {
        ...
        for (var i = 0, il = R.PICS.length; i < il; i++) {
            var pic = R.PICS[i];
            img = pic.img;
            iw = pic.origWidth;
            ih = pic.origHeight;
    ...
    loadPage: function (page) {
        ...
        $.ajax({
            url: PICWALL.CONFIG.picURL,
            data: data || {},
            type:"POST",
            dataType: 'json',
            success: function (res) {
                if (res.code === 200) {
                    picResult.call(this, res.data);
                }
            },
            error: function () {
                alert('网络出错，请重试');
    ...
    calcTime: function (ts) {
        //var temp=new Date(Date.parse(ts.replace(/-/g, "/")));
        var strArray=ts.split(" ");
        var strDate=strArray[0].split("-");
        var strTime=strArray[1].split(":");
        var t1=new Date(strDate[0],(strDate[1]-parseInt(1)),+strDate[2],+strTime[0],+strTime[1],+strTime[2])
        var C = PICWALL.CONFIG,
            dur,
            t2 = new Date();
        //t1.setTime(temp.getMilliseconds());
